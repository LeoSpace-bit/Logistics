"""Заполнение базы данных: 21 пункт, транспортная сеть, пользователи."""
...
def _bidir(
    from_id: int, to_id: int, tt: str, dist: int, dur: int, cost: str,
    *,
    max_w: float | None = None, max_v: float | None = None,
    dangerous: bool = False, fragile: bool = True,
    liquid: bool = True, perishable: bool = True,
    crushable: bool = True, temp_ctrl: bool = False,
) -> list[TransportLinkORM]:
    common = dict(
        transport_type=tt, distance_km=dist, duration_min=dur,
        cost_base=Decimal(cost),
        max_weight_kg=max_w, max_volume_m3=max_v,
        allows_dangerous=dangerous, allows_fragile=fragile,
        allows_liquid=liquid, allows_perishable=perishable,
        allows_crushable=crushable, allows_temp_control=temp_ctrl,
    )
    return [
...
LOCATIONS: list[dict] = [
    ...
    # ── НОВЫЙ: Псков — изолированный узел, доступен ТОЛЬКО авиа из Сочи ──
    dict(id=21, name="Псков ПВЗ",             type="PICKUP_POINT",  address="г. Псков, Октябрьский пр., 12",                   geo_lat=57.8136, geo_lon=28.3496),
]
...
def _build_links() -> list[TransportLinkORM]:
    ...
    rail = [
        ...
        (5,  6,  1500, 1440,  "4000"),
        (6,  11,  800,  720,  "2600"),
        (11, 12, 1100, 1080,  "3100"),
        (12, 13, 3500, 2880,  "7500"),
        (2,  16, 1400, 1440,  "4200"),
        (3,  18,  560,  480,  "2000"),
    ]
    ...
    # ── AIR ───────────────────────────────────────────────────────────
    # Авиа: жидкости и опасные — ЗАПРЕЩЕНЫ
    ...
    air = [
        ...
        (1, 11,  3400,  270, "15500"),
        (6, 14,  4400,  360, "19000"),
        # ── Псков: ТОЛЬКО авиа из Сочи (тупиковый узел) ──
        (17, 21, 1800,  180,  "9500"),
    ]
    ...
    # ── SEA ───────────────────────────────────────────────────────────
    # Хрупкие, мнущиеся, скоропортящиеся — ЗАПРЕЩЕНЫ
