"""SQLAlchemy ORM — декларативные модели таблиц (ERD → код)."""
...
class UserORM(Base):
    ...
    sent_orders: Mapped[list[OrderORM]] = relationship(
        ...
        foreign_keys="OrderORM.sender_id",
    )
...
class TransportLinkORM(Base):
    ...
    cost_base: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False,
    )
    max_weight_kg: Mapped[float | None] = mapped_column(
        Float, nullable=True,
    )
    max_volume_m3: Mapped[float | None] = mapped_column(
        Float, nullable=True,
    )
    ...
    from_location: Mapped[LocationORM] = relationship(
        foreign_keys=[from_location_id],
    )
    to_location: Mapped[LocationORM] = relationship(
...
class OrderORM(Base):
    ...
    sender: Mapped[UserORM] = relationship(
        back_populates="sent_orders", foreign_keys=[sender_id],
    )
    receiver: Mapped[UserORM | None] = relationship(
        foreign_keys=[receiver_id],
    )
    origin: Mapped[LocationORM] = relationship(
        foreign_keys=[origin_location_id],
    )
    destination: Mapped[LocationORM] = relationship(
        foreign_keys=[dest_location_id],
    )
    cargo: Mapped[CargoORM | None] = relationship(
        back_populates="order", uselist=False,
    )
    route_segments: Mapped[list[OrderRouteSegmentORM]] = relationship(
        back_populates="order",
    )
    tracking_history: Mapped[list[TrackingHistoryORM]] = relationship(
        back_populates="order",
    )
...
class OrderRouteSegmentORM(Base):
    ...
    link_id: Mapped[int] = mapped_column(
        ForeignKey("transport_links.id"), nullable=False,
    )
